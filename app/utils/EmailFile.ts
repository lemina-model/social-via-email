 ...
 * Uses the APP_KEYWORD Gmail label as the "folder"; the email subject is the "name".
 */
...
/** Resolve the Gmail label ID for APP_KEYWORD. */
async function getAppLabelId(accessToken: string): Promise<string | null> {
  ...
  const label = labels.find((l) => l.name === APP_KEYWORD);
...
/**
 * Read an email from the APP_KEYWORD folder whose subject equals `name`.
...
/**
 * Write an email into the APP_KEYWORD folder with subject `name` and body `content`.
...
export async function writeEmail(
  ...
  const labelId = await getAppLabelId(accessToken);
  if (!labelId) throw new Error(`Label "${APP_KEYWORD}" not found`);
  ...
  const lines = [
    `From: storage@${APP_KEYWORD}.local`,
    `To: storage@${APP_KEYWORD}.local`,
